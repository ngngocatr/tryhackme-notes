...
## 4. Process
Chúng ta đã thảo luận về vai trò và trách nhiệm của các cá nhân khác nhau làm việc trong nhóm SOC . Mỗi vai trò đều có quy trình riêng , giống như vai trò của các chuyên viên phân tích SOC cấp 1 là những người phản hồi đầu tiên, thực hiện phân loại cảnh báo và xác định xem chúng có gây hại hay không. Hãy cùng thảo luận về một số quy trình quan trọng liên quan đến SOC .

### 1. Alert Triage(_Phân loại cảnh báo_)
Phân loại cảnh báo là nền tảng của nhóm SOC . Phản hồi đầu tiên đối với bất kỳ cảnh báo nào là thực hiện phân loại. Việc phân loại tập trung vào việc phân tích cảnh báo cụ thể. Điều này xác định mức độ nghiêm trọng của cảnh báo và giúp chúng ta ưu tiên nó. Phân loại cảnh báo xoay quanh việc trả lời 5 câu hỏi W. Vậy 5 câu hỏi W đó là gì?

<img src="../../images/2026-02-12-08-36-43.png" style="display: block; margin: 0 auto;">

Sau đây là một số câu hỏi cần được trả lời trong quá trình phân loại cảnh báo. 

Cảnh báo: Phát hiện phần mềm độc hại trên máy chủ: **GEORGE PC**

<img src="../../images/2026-02-12-08-38-02.png" style="display: block; margin: 0 auto;">

### 2. Reporting
Các cảnh báo nguy hiểm được phát hiện cần được chuyển tiếp đến các nhà phân tích cấp cao hơn để có phản hồi và giải quyết kịp thời. Các cảnh báo này được chuyển thành phiếu yêu cầu và được chỉ định cho người có trách nhiệm. Báo cáo cần thảo luận đầy đủ về cả 5 yếu tố W (Who, What, When, Why) cùng với phân tích chi tiết, và cần sử dụng ảnh chụp màn hình làm bằng chứng về hoạt động đó.

### 3. Incident Response and Forensics
Đôi khi, các báo cáo phát hiện chỉ ra các hoạt động độc hại nghiêm trọng. Trong những trường hợp này, các nhóm cấp cao sẽ khởi động quy trình ứng phó sự cố. Quy trình ứng phó sự cố được thảo luận chi tiết trong  phòng Ứng phó sự cố . Một vài lần, hoạt động điều tra pháp y chi tiết cũng cần được thực hiện. Hoạt động điều tra pháp y này nhằm mục đích xác định nguyên nhân gốc rễ của sự cố bằng cách phân tích các bằng chứng từ hệ thống hoặc mạng

## 5. Technology
Việc chỉ có đúng **người** và **quy trình** là chưa đủ nếu thiếu các giải pháp bảo mật để phát hiện và ứng phó. Phần công nghệ  trong các trụ cột của SOC đề cập đến các giải pháp bảo mật. Những giải pháp bảo mật này giúp giảm thiểu công sức thủ công của nhóm SOC trong việc phát hiện và ứng phó với các mối đe dọa.

Mạng lưới của một tổ chức bao gồm nhiều thiết bị và ứng dụng. Đối với đội ngũ bảo mật, việc phát hiện và ứng phó riêng lẻ với các mối đe dọa trên từng thiết bị hoặc ứng dụng sẽ đòi hỏi rất nhiều công sức và nguồn lực. Các giải pháp bảo mật tập trung hóa tất cả thông tin về các thiết bị hoặc ứng dụng hiện có trong mạng và tự động hóa khả năng phát hiện và ứng phó.

Hãy cùng tìm hiểu sơ lược về một số giải pháp bảo mật này:
1. **SIEM**: **Security Information and Event Management** là một công cụ phổ biến được sử dụng trong hầu hết mọi môi trường SOC . Công cụ này thu thập nhật ký từ nhiều thiết bị mạng khác nhau, được gọi là nguồn nhật ký. Các quy tắc phát hiện được cấu hình trong giải pháp SIEM , chứa logic để xác định hoạt động đáng ngờ. Giải pháp SIEM cung cấp cho chúng ta các phát hiện sau khi đối chiếu chúng với nhiều nguồn nhật ký và cảnh báo chúng ta trong trường hợp trùng khớp với bất kỳ quy tắc nào. Các giải pháp SIEM hiện đại vượt trội hơn so với phân tích phát hiện dựa trên quy tắc này, cung cấp cho chúng ta khả năng phân tích hành vi người dùng và khả năng thu thập thông tin tình báo về mối đe dọa. Các thuật toán học máy hỗ trợ điều này để tăng cường khả năng phát hiện.

_*Lưu ý_: Giải pháp SIEM chỉ cung cấp khả năng phát hiện trong môi trường SOC .

2. **EDR**: Endpoint Detection and Response  cung cấp cho nhóm SOC khả năng hiển thị chi tiết theo thời gian thực và lịch sử hoạt động của các thiết bị. Nó hoạt động ở cấp độ điểm cuối và có thể thực hiện các phản hồi tự động. EDR có khả năng phát hiện điểm cuối rộng rãi, cho phép bạn điều tra chúng một cách chi tiết và phản hồi chỉ với vài cú nhấp chuột.

3. **Firewall**: Tường lửa có chức năng thuần túy là bảo mật mạng và hoạt động như một rào cản giữa mạng nội bộ và mạng bên ngoài (như Internet). Nó giám sát lưu lượng mạng đến và đi, đồng thời lọc bỏ mọi lưu lượng truy cập trái phép. Tường lửa cũng có một số quy tắc phát hiện được triển khai, giúp chúng ta xác định và chặn lưu lượng truy cập đáng ngờ trước khi nó đến mạng nội bộ.

Một số giải pháp bảo mật khác cũng đóng vai trò riêng biệt trong môi trường SOC , chẳng hạn như _phần mềm chống virus, EPP , IDS / IPS , XDR, SOAR , và nhiều hơn nữa_. Quyết định về công nghệ nào sẽ được triển khai trong SOC được đưa ra sau khi cân nhắc kỹ lưỡng về bề mặt mối đe dọa và các nguồn lực sẵn có trong tổ chức.
